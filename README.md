...
Хранение и просмотр аватаров и файлов будет осуществляться с помощью [Amazon S3 standard storage](https://aws.amazon.com/s3/?did=ft_card&trk=ft_card).

Микросервис профиля будет получать аватарку от пользователя, сохранять в Amazon S3 и записывать ссылку в БД

Микросервис сообщений будет получать файл от пользователя, сохранять в Amazon S3 и записывать ссылку в БД.

Подсчитаем общий размер хранимых данных, используя данные суточного траффика:
1. Аватарки: 136.72 ГБ = 0.15 ТБ/сутки => 0.15 * 365 * 3 = 164 ТБ
2. Файлы: 19824.22 ГБ = 20 ТБ/сутки => 20 * 365 * 3 = 21900 ТБ
3. Итого: **22 ПБ** файлов
4. Хранилище должно хранить, с учётом запаса, **30 ПБ**

Основное хранилище разместим в Северной Америке, т.к. там больше всего пользователей. 
...
* В каждый сервер поместим по 8 HDD => (8 / 2) * 18 = 72 ТБ
...
Тогда нам потребуется 2420 / 72 = **34 сервера**
...
| Сервер             | Всего  | Северная Америка | Южная Америка | Европа | Азия   |
| :----:             | :----: | :----:           | :----:        | :----: | :----: | 
| Хранилище файлов   | 2    | 1 (AS3)| - | 1 (AS3 repl) | - |
| Postgres           | 10   | 2   | 2   | 2   | 2   |
| Голосовые сервера  | 814  | 244 | 143 | 143 | 143 |
| Сервера трансляций | 3037 | 911 | 532 | 532 | 532 |
| Сервера сообщений  | | | | 
| Nginx              | | | | 
| Микросервисы       | | | |
